...
if (!MONGO_URL) {
    throw new Error('MONGO_URL environment variable is not defined. Please create a .env file with MONGO_URL.');
}
...
export async function deleteMeeting(meetingId, userId = null) {
    ...
    if (tRes.deletedCount > 0) {
        cRes = await database.collection('chunks').deleteMany({ meetingId });
    }
