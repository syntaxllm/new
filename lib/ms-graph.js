...
export async function listRecentMeetings(accessToken) {
    ...
    async function fetchOneDrivePathRecordings() {
        ...
        const paths = ['/me/drive/root:/Recordings:', '/me/drive/special/recordings', '/me/drive/root:/recordings:'];
        for (const pathPrefix of paths) {
            try {
                const url = `https://graph.microsoft.com/v1.0${pathPrefix}/children?$top=100&$orderby=createdDateTime desc`;
                const res = await fetch(url, { headers: { 'Authorization': `Bearer ${accessToken}` } });
                if (res.ok) {
                    const data = await res.json();
                    const files = (data.value || [])
                    ...
                    console.log(`[Discovery] Found ${files.length} items in path: ${pathPrefix}`);
                    allFiles = [...allFiles, ...files];
                }
            } catch (e) { /* ignore */ }
        }
