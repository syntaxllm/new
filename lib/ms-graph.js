...
export async function listRecentMeetings(accessToken) {
    ...
    async function fetchGlobalMeetingSearch() {
        ...
        // Only look for recordings from the last 7 days to keep results relevant
        const dateThreshold = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString();
            ...
            requests: [
                {
                    ...
                    query: {
                        // Broaden query: Any MP4 or VTT modified recently
                        queryString: `(filetype:vtt OR filetype:mp4) AND createdDateTime >= ${dateThreshold}`
                    },
                    from: 0,
                    size: 30
                }
        ...
        try {
            ...
            console.log(`[Global Search] Found ${hits.length} recent files`);
    ...
    // Helper: Recursive scan for "Recordings" folder and its subfolders (e.g. "Recordings/data")
    async function fetchOneDrivePathRecordings() {
        ...
        for (const pathPrefix of paths) {
            try {
                const url = `https://graph.microsoft.com/v1.0${pathPrefix}/children?$top=50&$expand=thumbnails`;
                const res = await fetch(url, { headers: { 'Authorization': `Bearer ${accessToken}` } });
                if (res.ok) {
                    const data = await res.json();
                    for (const item of (data.value || [])) {
                        // 1. Handle Files in the folder
                        if (item.file && (item.name.toLowerCase().endsWith('.mp4') || item.name.toLowerCase().endsWith('.vtt'))) {
                            allFiles.push({
                                id: item.id,
                                ...
                                resourcePath: item.parentReference?.driveId ? `/drives/${item.parentReference.driveId}/items/${item.id}` : `/me/drive/items/${item.id}`
                            });
                        }
                        // 2. Deep Scan Subfolders (like "data")
                        if (item.folder) {
                            console.log(`[OneDrive] Deep scanning subfolder: ${item.name}`);
                            try {
                                const subRes = await fetch(`https://graph.microsoft.com/v1.0/me/drive/items/${item.id}/children`, { headers: { 'Authorization': `Bearer ${accessToken}` } });
                                if (subRes.ok) {
                                    const subData = await subRes.json();
                                    for (const subItem of (subData.value || [])) {
                                        if (subItem.file && (subItem.name.toLowerCase().endsWith('.mp4') || subItem.name.toLowerCase().endsWith('.vtt'))) {
                                            allFiles.push({
                                                id: subItem.id,
                                                driveId: subItem.parentReference?.driveId,
                                                subject: subItem.name,
                                                start: subItem.createdDateTime,
                                                end: subItem.lastModifiedDateTime,
                                                webUrl: subItem.webUrl,
                                                downloadUrl: subItem['@microsoft.graph.downloadUrl'],
                                                driveItemId: subItem.id,
                                                onlineMeetingId: null,
                                                isOrganizer: true,
                                                source: 'onedrive',
                                                isVttFile: subItem.name.toLowerCase().endsWith('.vtt'),
                                                resourcePath: subItem.parentReference?.driveId ? `/drives/${subItem.parentReference.driveId}/items/${subItem.id}` : `/me/drive/items/${subItem.id}`
                                            });
                                        }
                                    }
                                }
                            } catch (err) { /* ignore subfolder failures */ }
                        }
                    }
                }
            } catch (e) { /* ignore */ }
    ...
    // Helper: Find recordings shared with me (if not organizer)
    async function fetchSharedWithMe() {
        ...
        try {
            ...
            const data = await res.json();
            return (data.value || [])
                .filter(item => item.file && (item.name.toLowerCase().endsWith('.mp4') || item.name.toLowerCase().endsWith('.vtt')))
                .map(item => ({
                    id: item.id,
                    ...
                    source: 'onedrive',
                    isVttFile: item.name.toLowerCase().endsWith('.vtt'),
                    resourcePath: (item.remoteItem?.parentReference?.driveId) ? `/drives/${item.remoteItem.parentReference.driveId}/items/${item.remoteItem.id}` : `/me/drive/items/${item.id}`
                }));
        } catch (e) { return []; }
    }

    const [calendarMeetings, onlineMeetings, driveRecordings, pathRecordings, searchRecordings, sharedRecordings] = await Promise.all([
    ...
    // Merge and Deduplicate with a focus on RECORDINGS
    const unique = new Map();
    const recordingThreshold = new Date(now.getTime() - 14 * 24 * 60 * 60 * 1000); // 14 days

    // 1. Process files first (OneDrive, Search, Shared)
    // Anything found in a physical OneDrive/SharePoint location is high-value
    const fileSources = [...searchRecordings, ...pathRecordings, ...driveRecordings, ...sharedRecordings];
    for (const m of fileSources) {
        // Broaden filter: If it's in a recordings folder OR modified recently, we show it
        // We trust the path-based discovery more than keyword search now
        unique.set(m.webUrl || m.id, m);
    }

    // 2. Add Online Meetings ONLY if they are very recent
    ...
    for (const m of apiSources) {
        ...
        // For API sources, we ONLY add them if they aren't already covered by a File
