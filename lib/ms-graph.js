...
export async function fetchUpcomingMeetings(accessToken) {
    const now = new Date();
    const endOfDay = new Date(now);
    endOfDay.setHours(23, 59, 59, 999);

    try {
        // Look from Now - 15m to End of Day
        const start = new Date(now.getTime() - 15 * 60000).toISOString();
        const end = endOfDay.toISOString();

        console.log(`[Graph API] Fetching active meetings from ${start} to ${end}`);

        // We use raw fetch here to keep it simple
        const url = `https://graph.microsoft.com/v1.0/me/calendarView?startDateTime=${start}&endDateTime=${end}&$filter=isOnlineMeeting eq true&$orderby=start/dateTime&$top=10`;
        const res = await fetch(url, { headers: { 'Authorization': `Bearer ${accessToken}` } });

        if (!res.ok) {
            console.error('[Graph API] Failed to fetch calendar:', res.status, res.statusText);
            return [];
        }
        ...
        return (data.value || []).map(evt => ({
            ...
            webUrl: evt.joinWebUrl, // The link the bot needs!
            organizer: evt.organizer.emailAddress.name,
            isCurrent: new Date(evt.start.dateTime) <= new Date() && new Date(evt.end.dateTime) >= new Date()
        }));
    } catch (error) {
        console.error('[Graph API] Failed to fetch upcoming meetings:', error);
        return [];
    }
}
 ...
 */
export async function extractMeetingIdFromRecording(accessToken, recordingName, siteId, driveId, folderId) {
    try {
