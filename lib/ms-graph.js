...
export async function listRecentMeetings(accessToken) {
    ...
    for (const m of fileSources) {
        ...
        // Actionable check: Trust files in "Recordings" folders OR with meeting-like names
        const isLikelyRecording = subjectLower.includes('recording') ||
            subjectLower.includes('call with') ||
            subjectLower.includes('meeting') ||
            m.resourcePath?.toLowerCase().includes('recordings');
