...
export async function listRecentMeetings(accessToken) {
    ...
    async function fetchOneDriveRecordings() {
        // We go directly to the path the user specified: OneDrive > My Files > Recordings
        ...
        try {
            ...
            if (!res.ok) {
                // Return empty instead of crashing or showing 400
                console.warn(`[OneDrive] No folder found at root:/Recordings (Status: ${res.status})`);
                return [];
            }

            const data = await res.json();
            console.log(`[OneDrive] Found ${data.value?.length || 0} direct items in /Recordings`);

            return (data.value || [])
                .filter(f => {
                    const name = (f.name || '').toLowerCase();
                    return f.file && (name.endsWith('.mp4') || name.endsWith('.vtt'));
                })
                .map(f => ({
                    id: f.id,
            ...
            console.warn('OneDrive direct /Recordings fetch failed', e);
            return [];
        }
    }
    ...
    for (const m of fileSources) {
        ...
        // NUCLEAR FILTER: No extension = No appearance. 
        if (!subjectLower.endsWith('.mp4') && !subjectLower.endsWith('.vtt')) {
            continue;
        }

        // Actionable check: For search hits, still ensure they look like meetings
        // But for direct folder hits (path/drive), we trust them 100%.
        const isFromExplicitFolder = m.resourcePath?.toLowerCase().includes('recordings');
        const isSearchHitActionable = subjectLower.includes('recording') ||
            subjectLower.includes('call with') ||
            subjectLower.includes('meeting');

        if (isFromExplicitFolder || isSearchHitActionable || m.isVttFile) {
            console.log(`[Deduplication] Including actionable file: ${m.subject}`);
