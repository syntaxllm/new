...
export async function listRecentMeetings(accessToken) {
    ...
    async function fetchOneDriveRecordings() {
        ...
        try {
            ...
            // Match VTT files to their MP4 counterparts
            const vttMap = new Map();
            const mp4Files = [];

            for (const file of allFiles) {
                if (file.isVttFile) {
                    // Extract base name (e.g., "Meeting-20260122.vtt" -> "Meeting-20260122")
                    const baseName = file.subject.replace(/\.vtt$/i, '');
                    vttMap.set(baseName.toLowerCase(), file);
                } else {
                    mp4Files.push(file);
                }
            }

            // Attach VTT paths to MP4 files
            for (const mp4 of mp4Files) {
                const baseName = mp4.subject.replace(/\.mp4$/i, '');
                const vttFile = vttMap.get(baseName.toLowerCase());

                if (vttFile) {
                    console.log(`[Discovery] Matched VTT for ${mp4.subject}: ${vttFile.subject}`);
                    mp4.vttResourcePath = vttFile.resourcePath;
                    mp4.vttDownloadUrl = vttFile.downloadUrl;
                    mp4.hasTranscript = true;
                }
            }
