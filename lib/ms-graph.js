...
export async function listRecentMeetings(accessToken) {
            ...
            requests: [
                {
                    ...
                    query: {
                        // Search for MP4/VTT that either have "Recording" in name OR are in a "Recordings" folder
    ...
    async function fetchOneDrivePathRecordings() {
        ...
        // 1. Try to find the Recordings folder first to get its ID (works across various drive locations)
        try {
            const searchUrl = `https://graph.microsoft.com/v1.0/me/drive/root/search(q='Recordings')?$filter=folder ne null`;
            const folderRes = await fetch(searchUrl, { headers: { 'Authorization': `Bearer ${accessToken}` } });
            if (folderRes.ok) {
                const searchData = await folderRes.json();
                const recordingsFolders = searchData.value || [];
                for (const folder of recordingsFolders) {
                    ...
                    if (res.ok) {
                        const data = await res.json();
                        const files = (data.value || []).filter(f => f.file).map(f => ({
                            id: f.id,
                            driveId: f.parentReference?.driveId,
                            subject: f.name,
                            start: f.createdDateTime,
        ...
        // 2. Fallback: Try multiple common paths for recordings
        ...
        for (const pathPrefix of paths) {
            try {
                ...
                if (res.ok) {
                    const data = await res.json();
                    const files = (data.value || []).filter(f => f.file && (f.name.toLowerCase().endsWith('.mp4') || f.name.toLowerCase().endsWith('.vtt'))).map(f => ({
                        id: f.id,
                        driveId: f.parentReference?.driveId,
                        subject: f.name,
                        start: f.createdDateTime,
        ...
        try {
            ...
            if (sharedRes.ok) {
                ...
                const recordings = (sharedData.value || [])
                    .filter(f => {
                        const name = (f.name || f.remoteItem?.name || '').toLowerCase();
                        return (f.file || f.remoteItem?.file) && (name.includes('recording') || name.includes('call with') || name.endsWith('.vtt'));
                    })
                    .map(f => {
    ...
    // Merge and Deduplicate with a strict focus on RECORDINGS
    ...
    for (const m of fileSources) {
        // Strict file filtering: Only relevant MP4 or VTT
        const subjectLower = m.subject?.toLowerCase() || '';
        const isActionable = subjectLower.includes('recording') ||
            subjectLower.includes('call with') ||
            m.isVttFile;
