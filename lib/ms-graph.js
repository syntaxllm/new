...
export async function listRecentMeetings(accessToken) {
    ...
    const [calendarMeetings, onlineMeetings, driveRecordings, searchRecordings] = await Promise.all([
    ...
    console.log(`[Discovery Stats] Calendar: ${calendarMeetings.length}, Online: ${onlineMeetings.length}, Drive: ${driveRecordings.length}, Search: ${searchRecordings.length}`);
    ...
    const fileSources = [...searchRecordings, ...driveRecordings];
