 ...
 */
export async function ingestBotTranscript(externalMeetingId, rawTranscript, metadata = {}) {
  if (!rawTranscript || rawTranscript.trim().length === 0) return;

  // 1. Normalize into meeting structure
  // We use the external ID provided by Teams to keep it consistent
  const meetingId = `bot-${externalMeetingId.substring(0, 12)}`;

  // Split raw transcript into chunks for display (e.g. by sentence/paragraph)
  const paragraphs = rawTranscript.split('\n').filter(p => p.trim().length > 0);
  const entries = paragraphs.map((p, idx) => ({
    speaker: metadata.subject || 'Meeting Participant',
    text: p.trim(),
    start: '00:00:00', // We don't have precise timings for bot live text yet
    end: '00:00:30'
  }));
  ...
  // 3. Update RAG Chunks so user can ask questions in real-time
  return processChunksAndSave(meetingObj, entries);
}

// Helper to chunk and save (unchanged, just ensuring it's available)
