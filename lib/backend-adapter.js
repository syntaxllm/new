 ...
 */
export async function ingestTeamsMeeting(accessToken, teamsMeetingId, resourcePath = null) {
  let vttContent;
  let source = 'Microsoft Teams API';

  // 1. Fetch real VTT content
  try {
    // If we have a resourcePath and it's a VTT file, fetch it directly
    if (resourcePath && resourcePath.toLowerCase().endsWith('.vtt')) {
      console.log(`[Ingest] Directly fetching VTT file from: ${resourcePath}`);
      vttContent = await fetchOneDriveFileContent(accessToken, null, resourcePath);
      source = 'OneDrive File';
    } else if (resourcePath && resourcePath.includes('/items/')) {
      // It's a drive item (likely MP4), try video transcript extraction
      console.log(`[Ingest] Detected Drive Item. Attempting Video Deep Scan: ${resourcePath}`);
      vttContent = await fetchVideoTranscript(accessToken, teamsMeetingId, resourcePath);
      source = 'Microsoft Stream (Video)';
    } else {
      // Normal Teams Meeting ID
      vttContent = await fetchTeamsTranscript(accessToken, teamsMeetingId);
    }
  } catch (err) {
    // Fallback if resourcePath wasn't provided or failed
    if (!vttContent && resourcePath) {
      console.warn(`[Ingest] Direct fetch failed for ${resourcePath}, falling back...`);
    }
    ...
    if (!vttContent) {
      try {
        ...
        if (e.message === 'NO_TRANSCRIPTS_FOR_MEETING' || e.message.includes('404') || e.message.includes('400')) {
          console.log(`[Ingest] No standard transcript found for ${teamsMeetingId}. Attempting Video Deep Scan...`);
          vttContent = await fetchVideoTranscript(accessToken, teamsMeetingId, resourcePath);
          source = 'Microsoft Stream (Video)';
        } else {
          throw e;
