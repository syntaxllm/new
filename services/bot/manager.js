...
class BotManager extends EventEmitter {
    constructor() {
        ...
        // Circular reference protection for global attachment
        if (typeof global !== 'undefined') {
            global.botManagerInstance = this;
        }
    }
    ...
    getAllBots() {
        ...
        return Array.from(this.bots.values()).map(b => {
            ...
            // Add participants list for UI pills
            safeBot.participants = [...new Set(b.transcriptSegments?.map(s => s.speaker) || [])].filter(Boolean);
    ...
    async handleBotMessage(id, msg) {
        ...
        if (msg.type === 'status') {
            ...
            const vttContent = this.generateVTT(result.segments || [], session);
     ...
     */
    generateVTT(segments, session) {
        const lines = ['WEBVTT', ''];

        let speakerEvents = session ? (session.speakerEvents || []) : [];
        let subject = session?.metadata?.subject || 'Meeting Participant';
        ...
        segments.forEach((s, i) => {
            ...
            // Mutation: Save the stitched speaker back into the segment object
            // This ensures the live UI (which reads session.transcriptSegments) displays the correct speaker.
            s.speaker = speaker;
    ...
    async finalizeMeeting(id) {
        ...
        try {
            ...
            // Ensure one final storage/vectorization pass with full data FIRST
...
// Singleton Instance - Use global if available to survive HMR in Next.js dev
let instance;
if (typeof global !== 'undefined' && global.botManagerInstance) {
    instance = global.botManagerInstance;
} else {
    instance = new BotManager();
}

export const botManager = instance;
export { BotState };
