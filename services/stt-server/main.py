...
def is_hallucination(text: str) -> bool:
    ...
    # Phrases that indicate hallucinations if they match EXACTLY or as SUBSTRINGS
    ...
    # Single words that are almost always hallucinations when appearing alone
    ...
    # 1. Check exact matches for risky single words
    if clean in SINGLE_WORD_HALLUCINATIONS:
        return True
        
    # 2. Check phrase substrings
    for phrase in PHRASE_HALLUCINATIONS:
        if phrase in clean:
            return True

    words = clean.split()
    # 3. Check for repetitive single-word noise (e.g., "you you you")
    if len(words) >= 2:
        unique_words = set(words)
        if len(unique_words) == 1 and words[0] in SINGLE_WORD_HALLUCINATIONS:
            return True
...
def transcribe(
    ...
    filename = f"{meeting_id}_{int(time.time())}_{batch_num}.webm"
    file_path = os.path.join(storage_dir, filename)
    normalized_path = None
    
    try:
        with open(file_path, "wb") as f:
            # Sync read since we are in a 'def' thread-safe function
        ...
        # Removed fallback logic that transcribes pure silence and causes extreme hallucination
        ...
        # CLEANUP: Crucial to avoid disk space exhaustion
        try:
            if file_path and os.path.exists(file_path):
                os.remove(file_path)
            if normalized_path and os.path.exists(normalized_path) and normalized_path != file_path:
                os.remove(normalized_path)
        except Exception as cleanup_err:
            print(f"⚠️ [{meeting_id}] Cleanup error: {cleanup_err}")
